...
async function GAME() {
    ...
    const enemyattack:number=10;
    let enemyAttack:number;
    const enemyhealth:number=5;
    ...
    let potionchance:number=10;

    const enemyList:string[]=['elon ','mark',];
    ...
    do{
        ...
        if(menueResponse==='Play'){
            ...
            currentEnemy=enemyList[getRandomNumber(0,4)];playerHealth=playerhealth;
                ...
                do{
                    battleResponse=String((await inquirer.prompt(Questions[2])).battleInput);
                        ...
                        switch(battleResponse){
                            case 'Attack':{
                                let playerAttack=getRandomNumber(0,playerattack);
